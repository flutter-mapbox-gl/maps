...
class SymbolOptionsProvider implements SymbolOptionsSink {
  private final SymbolOptions symbolOptions;

    SymbolOptionsProvider() {
    this.symbolOptions = new SymbolOptions();
  }

  void commitToBatchCreateCommand(BatchSymbolsCreateCommand command) {
      command.addSymbolOptions(symbolOptions);
  }
